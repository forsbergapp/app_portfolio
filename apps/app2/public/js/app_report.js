...
const timetable_user_setting_get = async (user_setting_id, callBack) => {
	...
	await common.FFB ('DB_API', `/user_account_app_setting/${user_setting_id}?`, 'GET', 0, null, (err, result) => {
		if (err){
			...
			const settings = JSON.parse(result_obj.settings_json);
...
const convertnumberlocale = (numberstring, splitcharacter, locale) => {
    const left = Number((numberstring).substr(0, (numberstring).indexOf(splitcharacter))).toLocaleString(locale);
    let right;
    let suffix;
...
const calculateIqamat = (option, calculated_time) => {
	...
	switch (option){
		...
		case '5':{
			...
			const datetime = new Date(1970, 1, 1, timeString[0], timeString[1]);
		...
		case '8':{
			...
			const datetime = new Date(1970, 1, 1, parseInt(timeString[0]) + 1, add_minutes);
...
const displayMonth = (prayTimes, settings, item_id, year_class='') => {
	...
	const set_month_year_title = () => {
		...
		if (settings.calendartype=='GREGORIAN'){
			...
			if (settings.calendartype=='HIJRI'){
				...
				const title_date = common.getGregorian(new Array(year,month,1), 0);
	...
	const timetable_data = ()=>{
		...
		//DATA
		while (date < endDate) {
			const times = prayTimes.getTimes(date, [settings.gps_lat, settings.gps_long], timezone_offset, 0, settings.format);
			if (settings.calendartype=='GREGORIAN')
			...
			if (settings.calendartype=='HIJRI'){
				const display_date = common.getGregorian(new Array(year,month,times['day']), settings.hijri_adj);
...
const displayDay = (prayTimes, settings, item_id, user_settings) => {
	...
	const date_current = new Date(app_common.APP_GLOBAL['session_currentDate'].getFullYear(),app_common.APP_GLOBAL['session_currentDate'].getMonth(),app_common.APP_GLOBAL['session_currentDate'].getDate());
	...
	//TIMETABLE
	return `<div id='timetable_day' class='${settings.timetable_class} ${settings.theme_day} ${settings.arabic_script}' style='direction: ${settings.direction};'>
				<div id='timetable_header' class='display_font' style='${getstyle(settings.header_img_src, settings.header_align)}'>
					<div >${settings.header_txt1}</div>
					<div >${settings.header_txt2}</div>
					<div >${settings.header_txt3}</div>
	...
	await common.FFB ('DB_API', `/user_account_app_setting/user_account_id/${user_account_id}?`, 'GET', 0, null, (err, result) => {
		if (err)
			...
			for (let i = 0; i < json.count; i++) {
                ...
                const settings = JSON.parse(json.items[i].settings_json);
				user_settings.push(
					{
...
const report_exception = (error) => {
	if (typeof error !='undefined' && error !=''){
		...
		const message_divs = document.getElementById('common_dialogue_message').getElementsByTagName('div');
...
const init = async (parameters) => {
	...
	const decodedReportparameters = common.fromBase64(params.get('reportid'));
		...
		common.init_common(parameters).then(()=>{
			for (let i = 0; i < parameters.app.length; i++) {
				...
				if (parameters.app[i].parameter_name=='REGIONAL_DEFAULT_CALENDAR_LANG')
					REPORT_GLOBAL['regional_def_calendar_lang'] = parameters.app[i].parameter_value;
				if (parameters.app[i].parameter_name=='REGIONAL_DEFAULT_LOCALE_EXT_PREFIX')
					REPORT_GLOBAL['regional_def_locale_ext_prefix'] = parameters.app[i].parameter_value;
				if (parameters.app[i].parameter_name=='REGIONAL_DEFAULT_LOCALE_EXT_NUMBER_SYSTEM')
					REPORT_GLOBAL['regional_def_locale_ext_number_system'] = parameters.app[i].parameter_value;
				if (parameters.app[i].parameter_name=='REGIONAL_DEFAULT_LOCALE_EXT_CALENDAR')
					REPORT_GLOBAL['regional_def_locale_ext_calendar'] = parameters.app[i].parameter_value;
				if (parameters.app[i].parameter_name=='REGIONAL_DEFAULT_CALENDAR_TYPE_GREG')
					REPORT_GLOBAL['regional_def_calendar_type_greg'] = parameters.app[i].parameter_value;
				if (parameters.app[i].parameter_name=='REGIONAL_DEFAULT_CALENDAR_NUMBER_SYSTEM')
					REPORT_GLOBAL['regional_def_calendar_number_system'] = parameters.app[i].parameter_value;
				//QR
									...
									if (reporttype==0){
										timetable_day_user_settings_get(user_account_id, (err, user_settings_parameters) =>{
											if (err)
												...
												document.getElementById('paper').innerHTML = displayDay(prayTimes, report_parameters, null, user_settings_parameters);
										...
										if (reporttype==1){
											document.getElementById('paper').innerHTML = displayMonth(prayTimes, report_parameters, null);
											common.create_qr('timetable_qr_code', window.location.href);
											resolve();
										}
										else 
											if (reporttype==2){
												document.getElementById('paper').innerHTML = displayYear(prayTimes, report_parameters, null);
												common.create_qr('timetable_qr_code', window.location.href);
												resolve();
											}
								});
