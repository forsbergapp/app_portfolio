...
|7.1.3      |2    |**Not implemented,  refactored OWASP security**.  Uses IAM pattern and is documented in OpenAPI and in Presentation App. Federated identity management ecosystem not allowed in project and not implemented since all users must be authenticated using same host only and must be able to be authenticated without using any network if necessary.|
...
|16.1.1      |2    |**Implemented**.  Uses OpenApi integrated with JSDoc documentation and added documentation in Presentation App that documents logging, what events are logged, log formats (JSON), where the logging is stored (ORM), how it is used, how access is controlled. Admin decides how long time to keep logs|
