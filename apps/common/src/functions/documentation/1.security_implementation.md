...
with Web Crypto API pattern saved in `/sdk/crypto.js` that is shared by app instances, server and microservices.
...
1.Replace old algorithms with post-quantum cryptograpyhy in `/server.security.js` if more than Web Crypto API should be modified.
2.Replace Web Crypto API implemented in shared `/sdk/crypto.js` if needed with new post-quantum cryptography.
