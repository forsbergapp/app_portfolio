...
JSDoc comment syntax and external types in Typescript *.ts files so no Typescript compiler is needed or any other Typescript npm module.
...
Javascript files *.js and Typescript files *.ts are checked.

Typescript configuration that is used by VSCodium without Typescript npm module:

`/tsconfig.json`

Installl typescript if needed and perform stateless type check 
...
## JSDoc and Typescript type checking and documentation

Project uses JSDoc tags for type checking and for documentation.

Types are declared in JSDoc comments both locally for simple types and in external Typescript *.ts files
that JSDoc comments read using import statement in JSDoc comment first in all modules.
...
|@typescript	|	        |used inside *.ts files                             |
...
Type declarations are saved in separate *.ts files. 
...
Example 1 - import types from Typescript
```
/&Star;&Star;
 * @import types_common  from './types.d.ts'
 */
 ```

Refering to a type using `@type` tag. 
Example 2 - @type syntax with imported types
```
/&Star;&Star;@type{types_common.COMMON_DOCUMENT} */
const COMMON_DOCUMENT = document;
```
Example 3 - @type syntax without imported types
```
...
```
Example 4 - third party
```
/&Star;&Star;@type{import('node:http').RequestOptions} */
const options = {
                            family: 4,
                            method: 'POST',
