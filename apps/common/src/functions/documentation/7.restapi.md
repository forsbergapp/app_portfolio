...
All REST API all called from Frontend For Backend (FFB) sending query and body in base64 format and decoded in server by Backend For Frontend (BFF). Body is sent using format {data:Object.<string,*>} converted to string and base64 format. Query is sent using parameters=base64 string. Body body and query formats managed by FFB and BFF.

All REST API use same request url with uuid and method if encryption is used. 

All REST API send data in body if encryption is used with route info including query, method and header.
