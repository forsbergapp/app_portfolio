...
#### Fetch tags on the application server
> **Note:** The commands here are only for the application server

Batch can do this automatically.
All tags must be deleted first since git pull does not sync deleted tags.
Command to delete local tags and update repository with commits and tags
```
git tag -l |xargs -n1 git tag -d && git pull --tags --force
```
