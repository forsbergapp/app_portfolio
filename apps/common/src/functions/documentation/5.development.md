...
Recreating a tag with date from old commit date (set env variables for each tag with the commit)
...
//Environment tag
git tag -a [TEST/PRODUCTION etc] $COMMIT -m ''
```
Setting a tag on latest commit (remove all env variables to use current date) and using changelog pattern and markdown
```
//Release tag
git tag -a [version tag] &grave;git merge-base refs/heads/main refs/remotes/origin/main&grave; -m '\n### Added\n- Description\n### Changed\n- Description\n### Fixed\n- Description\n### Removed\n- Description\n### Security\n- Description'
//Environment tag
git tag -a [TEST/PRODUCTION etc] &grave;git merge-base refs/heads/main refs/remotes/origin/main&grave; -m ''
```
Setting a tag on a specific commit (remove all env variables to use current date) and using changelog pattern and markdown.
...
Update, delete or add tags.
To sync added, updated and deleted tags, the remote tags must be deleted first if tags are deleted
```
...
To sync tags that are added or updated only
```
...
git fetch origin $ENVIRONMENT --tags && git reset --hard FETCH_HEAD && git tag -l |xargs -I T bash -c 'if [[ $(git log -1 "T" --format=%ci) > $(git log -1 --format=%ci) ]]; then echo git tag -d  "T"; fi' && git tag | grep -vxFf <(git ls-remote --tags origin| awk '{print $2}'|sed 's|refs/tags/||')|xargs git tag -d
```
