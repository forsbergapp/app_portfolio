...
ESLint
configuration file:
`$HOME/app_portfolio/.vscode/settings.json`
...
|Type                   |Comment                                    |
|:----------------------|:------------------------------------------|
|snake_case             |ISO20022 REST API parameters, Javascript parameters, HTML element id and classes, css selectors, filenames|
|SCREAMING_SNAKE_CASE	|constants|
|PascalCase				|ORM file names, ORM object names, ORM column names|
...
``` 
cd $HOME/app_portfolio
npx eslint --fix-dry-run '&Star;&Star;/*.js'
```
...
and consistency performance issue using replication. PM2 is not supported in localhost.
