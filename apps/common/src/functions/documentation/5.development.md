...
Uses tag descriptions for release information.
...
COMMIT_DATE=$(git show -s --format=%cd --date=iso8601-strict $COMMIT)
export GIT_AUTHOR_DATE="$COMMIT_DATE"
export GIT_COMMITTER_DATE="$COMMIT_DATE"
git tag -d [tag]
git tag -a [tag] $COMMIT  -m '\n### [keyword]\n- Description'
```
Setting a tag on latest commit (remove all env variables)
```
git tag -a [tag] -m '\n### [Keyword]\n- Description'
```
...
Command to fetch to given tag and fetch all tags, reset to fetched tag and delete tags newer than last commit
```
//replace $ENVIRONMENT with TEST, PROD or any tag
git fetch origin $ENVIRONMENT --tags && git reset --hard FETCH_HEAD && git tag -l |xargs -I T bash -c 'if [[ $(git log -1 "T" --format=%ci) > $(git log -1 --format=%ci) ]]; then echo git tag -d  "T"; fi'
```
