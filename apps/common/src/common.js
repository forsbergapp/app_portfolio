...
const commonAppStart = async (app_id=null) =>{
    ...
    if (fileCache('CONFIG_SERVER').METADATA.MAINTENANCE==0 && fileCache('CONFIG_SERVER').SERVICE_DB.filter((/**@type{*}*/key)=>key=='START')[0].START=='1' && 
...
const commonComponentCreate = async parameters =>{
    ...
    switch (parameters.type){
        case 'APP':{
            ...
            const ITEM_COMMON_PARAMETERS  = {APP_PARAMETERS:{   ...{app_id: parameters.app_id, ...fileCache('CONFIG_APPS').APPS[parameters.app_id].PARAMETERS},
                                                                ...{app_id: common_app_id, ...fileCache('CONFIG_APPS').APPS[common_app_id].PARAMETERS}},
                                            APP_SERVICE_PARAMETERS:app_service_parameters};
        ...
        case 'REPORT':{
            ...
            const module = Buffer.from(parameters.componentParameters.reportid ?? '', 'base64').toString('utf-8').split('&').filter(parameter=>parameter=='module')[0];
...
const commonApp = async parameters =>{
    ...
    const app_id = fileCache('CONFIG_APPS').APPS.filter((/**@type{*}*/app)=>host_no_port in app.SUBDOMAIN)[0].ID;
...
export {commonAssetfile,commonFunctionRun,commonModuleGet,commonApp, commonBFE};