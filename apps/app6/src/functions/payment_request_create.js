...
const payment_request_create = async (app_id, data, user_agent, ip, locale, res) =>{
	...
	if (data.currency_code==currency.currency_code && data.payerid !='' && data.payerid !=null){
        ...
        *          origin:         string}}
        */
        const body = {	api_secret:     fileDBGet(app_id, 'APP_SECRET',null, app_id)[0].merchant_api_secret,
                        reference:      data.reference.substring(0,30),
                        payeeid:        fileDBGet(app_id, 'APP_SECRET',null, app_id)[0].merchant_vpa, 
        ...
        const body_encrypted = {id:     fileDBGet(app_id, 'APP_SECRET',null, app_id)[0].merchant_id,
        ...
        if (result_commonBFE.error){
            ...
            */
            const body_decrypted = JSON.parse(securityPrivateDecrypt(
                                                    fileDBGet(app_id, 'APP_SECRET',null, app_id)[0].merchant_private_key, 
            ...
            return [{   token:                  body_decrypted.token,
                        ...
                        merchant_name:          fileDBGet(app_id, 'APP_SECRET',null, app_id)[0].merchant_name,
