...
const product_get = async (app_id, data, user_agent, ip, locale, res) =>{
    ...
    const products = await MasterGet(app_id, data.resource_id, 
    ...
    for (const product of products.rows ?? products){
        product.sku = [];
        const product_variants = await DetailGet(app_id, data.resource_id_variant, 
        ...
        for (const product_variant of product_variants.rows ?? product_variants){
            ...
            const product_variant_metadatas = await MasterGet(app_id, data.resource_id, 
