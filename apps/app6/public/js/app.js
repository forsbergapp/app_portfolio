...
const appProductUpdate = async () =>{
    ...
    await common.commonComponentRender({
        ...
        data:       {
                    ...
                    master_body:{type:'FUNCTION',IAM_data_app_id:common.commonGlobalGet('Data').UserApp.app_id, resource_id : product_variant_id},
                    master_method:'POST',
...
const appPaymentRequestStatus = ()=>{
    if ( new Date().getSeconds() % 2 && COMMON_DOCUMENT.querySelector('#common_app_dialogues_app_data_display .common_app_data_display_master_col2[data-key=PaymentRequestId]')?.getAttribute('data-value')){
        ...
        .then((/**@type{*}*/result)=>{
            ...
            if (status != 'PENDING'){
                common.commonGlobalSet({key:'Data', name:'token_external', value:null});
    ...
    if (appVPAIsValid(payerid_element, payerid_element.textContent)){
        ...
        await common.commonComponentRender({
            ...
            .then(result=>{
                ...
                common.commonGlobalSet({key:'Data', name:'token_external', value:result.data.master_object.Token.Value});
