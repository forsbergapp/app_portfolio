		...
		switch (parameters.data.model){
			case 0:{
				...
				if (solver2_moves_from_solved=='')
					...
					await socketAppServerFunctionSend(parameters.app_id, parameters.iam, 'PROGRESS', btoa(JSON.stringify({part:1, total:3, text:''})));
					...
					await socketAppServerFunctionSend(parameters.app_id, parameters.iam, 'PROGRESS', btoa(JSON.stringify({part:2, total:3, text:''})));
