...
RubiksCubeSolver.prototype.setState = function(cube) {
	...
	for(let i=0; i<20; i++){
		...
		//--- Current (start) state.
		let cubie = cube[i];
		while((this.currentState[i] = GOAL_SOLVE.indexOf(cubie)) == -1){
		...
		//GOAL_SOLVE[GOAL_SOLVE.indexOf(cubie)] = '';
	}
...
RubiksCubeSolver.prototype.solve = function(cube_currentstate, cube_goalstate) {
	...
	if(cube_currentstate){
		if(!this.setState(cube_currentstate))
			return false;
	}
...
/**
 * @param {number} app_id
 * @param {{cube_currentstate:string, cube_goalstate:string}} data
...
const cube_solve = async (app_id, data, user_agent, ip, locale, res) =>{
    ...
    const solution = solver.solve(data.cube_currentstate, data.cube_goalstate ?? GOAL_SOLVE);
    return [{cube_solution:solution}];
