...
const appUserLoginPost = async () =>{
    if (common.commonGlobalGet('Data').User.iam_user_id !=null){
        ...
        const result = await common.commonFFB({ path:'/server-iam/iamuserappdatapost/', 
                                                query:`IAM_data_app_id=${common.commonGlobalGet('Data').UserApp.app_id}&iam_user_id=${common.commonGlobalGet('Data').User.iam_user_id??''}`, 
                                                method:'GET', authorization_type:'APP_ID'});
         let settings = []
        if (JSON.parse(result).rows.length==0){
            await appUserSettingFunction('ADD_LOGIN', true);
            settings = await appUserSettingsGet({iam_user_id:common.commonGlobalGet('Data').User.iam_user_id});
        }
        else
            settings = await appUserSettingsGet({iam_user_id:common.commonGlobalGet('Data').User.iam_user_id, settings_fetched:result})
        common.commonComponentRemove('common_app_dialogues_app_custom')
...
const appUserProfileDetail = (detailchoice) => {
    if (common.commonGlobalGet('Data').User.iam_user_id || 0 !== 0) {
        if (detailchoice == 0){
            //user settings
    ...
    switch (item.id){
        ...
        case 'user_year_html':{
            ...
            appTimetablePreview({   type:item.id
                                            ...
                                            .replace('_html','')
                                            .toUpperCase(),
                                    iam_user_id:common.commonGlobalGet('Data').User.iam_user_id,
                                    sid:sid ??0,
                                    current:true
            })
            break;
    ...
    switch (choice){
        ...
        case 1:{
            common.commonFFB({  path:`/server-db/iamuserappdatapost/${user_setting_id}`, 
                                ...
                                body:{IAM_iam_user_app_id:common.commonGlobalGet('Data').UserApp.iam_user_app_id}, spinner_id:'setting_btn_user_delete'})
            .then(()=>{
...
const appUserSettingDefaultSet = async () => {
    ...
    const Document = {
        ...
        RegionalLanguageLocale:             common.commonGlobalGet('Data').UserApp.user_locale,
