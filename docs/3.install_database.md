...
# Install database
    ...
    Oracle
        ...
        Oracle Client installation
            --Needed for both docker installation and Oracle Cloud usage
            Ubuntu
        ...
        Docker installation
            ...
            -- https://localhost:8443/ords/my_atp/
            -- https://localhost:8443/ords/my_atp/sql-developer
            -- https://localhost:8443/ords/my_adw/
            -- https://localhost:8443/ords/my_adw/sql-developer

            Ubuntu
                sudo docker pull container-registry.oracle.com/database/adb-free:latest
                sudo docker run -d -p 1521:1522 -p 1522:1522 -p 8443:8443 -p 27017:27017 --hostname localhost --cap-add SYS_ADMIN --device /dev/fuse --name adb_container container-registry.oracle.com/database/adb-free:latest
                sudo docker exec adb_container /u01/scripts/change_expired_password.sh MY_ATP admin Welcome_MY_ATP_1234 [NEW PASSWORD]
                sudo docker exec adb_container /u01/scripts/change_expired_password.sh MY_ADW admin Welcome_MY_ADW_1234 [NEW PASSWORD]

                sudo docker cp container_id:/u01/app/oracle/wallets/tls_wallet/. $HOME/app_portfolio/config/oracle_instantclient/network/admin
                --serverparameters for Oracle Client to be set in /service/db/config/config.json (set full path without variables):
                    ...
                    DB4_CONNECTSTRING:  my_atp_high

                --enter filesystem when needed
                sudo docker exec -t -i adb_container bash
            Windows
                ...
                docker exec adb_container /u01/scripts/change_expired_password.sh MY_ATP admin Welcome_MY_ATP_1234 [NEW PASSWORD]
                docker exec adb_container /u01/scripts/change_expired_password.sh MY_ADW admin Welcome_MY_ADW_1234 [NEW PASSWORD]

                docker cp adb_container:/u01/app/oracle/wallets/tls_wallet/. %HOMEPATH%\app_portfolio\config\oracle_instantclient\network\admin
        ...
        Oracle Cloud installation
            Ubuntu
                ...
                --serverparameters for Oracle Client to be set in /service/db/config/config.json (set full path without variables):
                    DB4_LIBDIR:         [$HOME/app_portfolio/config/oracle_instantclient]
                    DB4_CONFIGDIR:      [$HOME/app_portfolio/config/oracle_instantclient/network/admin]
                    DB4_CONNECTSTRING:  [choose in tnsnames.ora]
            Windows
                --download wallet.zip from Oracle Cloud
