                                        ...
                                        return new Promise((resolve, reject) => {
                                            ...
                                            import(`file://${process.cwd()}/apps/apps.service.js`).then(({read_app_files}) => {
                                                read_app_files(files, (err, app_files)=>{
                                                    render_common_html(app_id, app_files, 'FORM', [map=true/false], [custom user_account div or NULL], [app_theme=true/false],
                                                                       [render_locales=true/false], [render_settings=true/false], [render_provider_buttons=true/false]).then((app)=>{
                                                        if (err)
                                                            reject(err);
                                                        else
                                                            resolve(app_init);
    ...
    /apps/app[APP_ID]/src/report/index.js   review /apps/[OTHER APP_ID]/src/report/index.js
                                        ...
                                        const getReport = (app_id, module, gps_lat, gps_long, gps_place) => {
                                            return new Promise((resolve, reject) => {
                                                ...
                                                import(`file://${process.cwd()}/apps/apps.service.js`).then(({read_app_files}) => {
                                                    render_common_html(app_id, app_files, 'REPORT', [map=false], [custom user_account div or NULL], [app_theme=false],
                                                                       [render_locales=false], [render_settings=false], [render_provider_buttons=false]).then((app)=>{
