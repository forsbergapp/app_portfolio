 ...
 */
const microserviceRequest = async (service, admin, path, query, data,  method,client_ip,authorization, headers_user_agent, headers_accept_language, server_app_timeout) =>{
    /**@type{import('./circuitbreaker.js')} */
    const {circuitBreaker} = await import(`file://${process.cwd()}/microservice/circuitbreaker.js`);

    return circuitBreaker.MicroServiceCall(microserviceHttpRequest, service, admin, path, query, data, method,client_ip,authorization, headers_user_agent, headers_accept_language, server_app_timeout);
}; 
