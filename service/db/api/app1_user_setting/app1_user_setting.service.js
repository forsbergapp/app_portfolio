...
module.exports = {
	createUserSetting: (app_id, initial, data, callBack) => {
		...
		if (process.env.SERVICE_DB_USE == 1) {
			sql = `INSERT INTO ${process.env.SERVICE_DB_DB1_NAME}.app1_user_setting(
						description,
			...
			parameters = [
							...
							data.regional_column_title,
							data.regional_arabic_script,
							data.regional_calendar_type,
							data.regional_calendar_hijri_type,
							data.gps_map_type,
							data.gps_country_id,
							data.gps_city_id,
							...
							data.design_paper_size,
							data.design_row_highlight,
							...
							data.prayer_method,
							data.prayer_asr_method,
							data.prayer_high_latitude_adjustment,
							data.prayer_time_format,
							data.prayer_hijri_date_adjustment,
							data.prayer_fajr_iqamat,
							data.prayer_dhuhr_iqamat,
							data.prayer_asr_iqamat,
							data.prayer_maghrib_iqamat,
							data.prayer_isha_iqamat,
							data.prayer_column_imsak_checked,
							data.prayer_column_sunset_checked,
							data.prayer_column_midnight_checked,
							data.prayer_column_fast_start_end,
			...
			sql = `INSERT INTO ${process.env.SERVICE_DB_DB2_NAME}.app1_user_setting(
						description,
						regional_language_locale,
						regional_current_timezone,
						regional_timezone,
	...
	updateUserSetting: (app_id, data, id, callBack) => {
		...
		let parameters;
		if (process.env.SERVICE_DB_USE == 1) {
			sql = `UPDATE ${process.env.SERVICE_DB_DB1_NAME}.app1_user_setting
					SET description = ?,
						regional_language_locale = ?,
						regional_current_timezone = ?,
						regional_timezone = ?,
			...
			sql = `UPDATE ${process.env.SERVICE_DB_DB2_NAME}.app1_user_setting
	...
	deleteUserSetting: (app_id, id, callBack) => {
		...
		let parameters;
		if (process.env.SERVICE_DB_USE == 1) {
			sql = `DELETE FROM ${process.env.SERVICE_DB_DB1_NAME}.app1_user_setting
					WHERE id = ? `;
			parameters = [id];
		}else if (process.env.SERVICE_DB_USE==2){
			sql = `DELETE FROM ${process.env.SERVICE_DB_DB2_NAME}.app1_user_setting
					WHERE id = :id `;
			parameters = {
							id: id
						 };
