...
module.exports = {
	...
	unlikeUserSetting: (app_id, id, id_unlike, callBack) => {
		...
		let parameters;
		if (process.env.SERVICE_DB_USE == 1) {
			sql = `DELETE FROM ${process.env.SERVICE_DB_DB1_NAME}.app2_user_setting_like
					WHERE user_account_id = ?
					AND app2_user_setting_id = ? `;
			parameters = [
							id,
							id_unlike
							];
		}else if (process.env.SERVICE_DB_USE==2){
			sql = `DELETE FROM ${process.env.SERVICE_DB_DB2_NAME}.app2_user_setting_like
