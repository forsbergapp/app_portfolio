 ...
 */
 const socketConnect = async parameters =>{
    ...
    const iam_user = serverUtilNumberValue(iamUtilDecode(parameters.iam).get('iam_user_id'))?
                    fileModelIamUser.get(parameters.app_id, serverUtilNumberValue(iamUtilDecode(parameters.iam).get('iam_user_id')), parameters.response)[0]:
                        null;
    const authorization_bearer = iamUtilDecode(parameters.iam).get('authorization_bearer');
    ...
    if (SOCKET_CONNECTED_CLIENTS.filter(row=>row.authorization_bearer == authorization_bearer).length>0 ||parameters.response.req.headers['sec-fetch-mode']!='cors'){
        ...
        const connectUserData =  await socketConnectedUserDataGet(parameters.app_id, user_account_id, parameters.ip, parameters.headers_user_agent, parameters.headers_accept_language, parameters.response);
        /**@type{server_socket_connected_list} */
        const newClient = {
                            id:                     client_id,
