...
resource "oci_core_instance" "instance" {
  ...
  metadata = {
    user_data               = base64encode(<<-EOF
                              ...
                              sudo -u ubuntu -H bash -c "$HOME/app_portfolio && /usr/bin/node server/init.js ${terraform.workspace} localhost ${var.environment_app_port} ${var.environment_admin_port}"
                              sleep 10
                              sudo reboot
                              EOF
                              )
