 ...
 * @returns {server['server']['response'] & {result?:server['ORM']['AppDataEntityResource'] & {app_data_name:string, app_data_value:string, app_data_display_data:string}[] }}
 */
const get = parameters =>{ 
    const result = (server.ORM.getObject(parameters.app_id, 'AppDataEntityResource',parameters.resource_id, null).result ?? [])
                    .filter((/**@type{server['ORM']['AppDataEntityResource']}*/row)=>
                                row.id                      == (parameters.resource_id ?? row.id) &&
                                row.app_data_entity_id      == (parameters.data.app_data_entity_id ?? row.app_data_entity_id));
    if (result)
        return {result:result
                        .map((/**@type{server['ORM']['AppDataEntityResource'] & {app_data_name:string, app_data_value:string, app_data_display_data:string}}*/row)=>{
                        ...
                        .filter((/**@type{server['ORM']['AppDataEntityResource'] & {app_data_name:string, app_data_value:string, app_data_display_data:string}}*/row)=>
 ...
 * @function
 * @param {{app_id:number,
 *          data:server['ORM']['AppDataEntityResource']}} parameters
...
const post = async parameters => {
   ...
   if (parameters.data.app_data_entity_id==null ||parameters.data.app_data_id==null){
       ...
       /**@type{server['ORM']['AppDataEntityResource']} */
       const data_new =     {
                                id:Date.now(),
       ...
       return server.ORM.Execute({app_id:parameters.app_id, dml:'POST', object:'AppDataEntityResource', post:{data:data_new}}).then((/**@type{server['ORMMetaData']['common_result_insert']}*/result)=>{
...
const update = async parameters =>{
   ...
   if (parameters.resource_id==null){
       ...
       if (Object.entries(data_update).length>0)
           return server.ORM.Execute({app_id:parameters.app_id, dml:'UPDATE', object:'AppDataEntityResource', update:{resource_id:parameters.resource_id, data_app_id:null, data:data_update}}).then((/**@type{server['ORMMetaData']['common_result_update']}*/result)=>{
               if (result.affectedRows>0)
                   return {result:result, type:'JSON'};
               else
                   return server.ORM.getError(parameters.app_id, 404);
...
const deleteRecord = async parameters =>{
    return server.ORM.Execute({app_id:parameters.app_id, dml:'DELETE', object:'AppDataEntityResource', delete:{resource_id:parameters.resource_id, data_app_id:null}}).then((/**@type{server['ORMMetaData']['common_result_delete']}*/result)=>{
        if (result.affectedRows>0)
            return {result:result, type:'JSON'};
        else
            return server.ORM.getError(parameters.app_id, 404);
