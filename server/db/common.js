 ...
 const dbCommonExecute = async (app_id, sql, parameters, dba = null, locale=null) =>{
	return new Promise ((resolve, reject)=>{
		...
		if (sql.indexOf('<APP_PAGINATION_LIMIT_OFFSET/>')>0){
			...
			if (!parameters.limit)
				parameters.limit = 	serverUtilNumberValue(fileModelAppParameter.get(serverUtilNumberValue(configGet('SERVER', 'APP_COMMON_APP_ID'))??0, null)[0].common_app_limit_records.value);
		}
		...
		if (sql.indexOf('<APP_LIMIT_RECORDS/>')>0){
			...
			parameters = {...parameters, ...{limit:serverUtilNumberValue(fileModelAppParameter.get(serverUtilNumberValue(configGet('SERVER', 'APP_COMMON_APP_ID'))??0, null)[0].common_app_limit_records.value)}};
		}
