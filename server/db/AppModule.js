 ...
 * @returns {server['server']['response'] & {result?:server['ORM']['AppModule'][] }}
 */
...
const post = async (app_id, data) => {
    ...
    if (app_id!=null && data.app_id!=null && data.ModuleType!=null && data.ModuleName!=null && data.ModuleRole!=null && data.ModulePath!=null){
        ...
        return server.ORM.Execute({app_id:app_id, dml:'POST', object:'AppModule', post:{data:data_new}}).then((/**@type{server['ORMMetaData']['common_result_insert']}*/result)=>{
 ...
 * @param {{app_id:number,
 *          resource_id:number,
 *          data:server['ORM']['AppModule']}} parameters
...
const update = async parameters => {
    ...
    if (Object.entries(data_update).length>0)
        return server.ORM.Execute({app_id:parameters.app_id, dml:'UPDATE', object:'AppModule', update:{resource_id:parameters.resource_id, data_app_id:null, data:data_update}}).then((/**@type{server['ORMMetaData']['common_result_update']}*/result)=>{
            if (result.affectedRows>0)
                return {result:result, type:'JSON'};
            else
...
const deleteRecord = async (app_id, resource_id) => {
    return server.ORM.Execute({app_id:app_id, dml:'DELETE', object:'AppModule', delete:{resource_id:resource_id, data_app_id:null}}).then((/**@type{server['ORMMetaData']['common_result_delete']}*/result)=>{
        if (result.affectedRows>0)
            return {result:result, type:'JSON'};
        else
            return server.ORM.getError(app_id, 404);
