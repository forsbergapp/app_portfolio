...
const post = async (app_id, data) => {
    ...
    if (app_id!=null && data.app_id!=null && data.ModuleType!=null && data.ModuleName!=null && data.ModuleRole!=null && data.ModulePath!=null){
        ...
        return server.ORM.Execute({app_id:app_id, dml:'POST', object:'AppModule', post:{data:data_new}});
    }
    else{
...
const update = async parameters => {
    ...
    if (Object.entries(data_update).length>0)
        return server.ORM.Execute({app_id:parameters.app_id, dml:'UPDATE', object:'AppModule', update:{resource_id:parameters.resource_id, data_app_id:null, data:data_update}});
    else
        return server.getError({statusCode: 400});
};
