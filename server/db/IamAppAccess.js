 ...
 * @function
 * @param {number} app_id 
 * @param {server['ORM']['IamAppAccess']} data
...
const post = async (app_id, data) =>{
    ...
    if (app_id!=null &&
        ...
        if (server.ORM.getObject(app_id, 'IamAppAccess', null, null).result.filter((/**@type{server['ORM']['IamAppAccess']} */row)=>row.token==data.token && data.token !=null).length==0){
            ...
            return server.ORM.Execute({app_id:app_id, dml:'POST', object:'IamAppAccess',post:{data:data_new}}).then((/**@type{server['ORMMetaData']['common_result_insert']}*/result)=>{
...
const update = async (app_id, resource_id, data) =>{
    ...
    if (app_id!=null && resource_id != null){
        ...
        if (Object.entries(data_update).length>1){
            /**@type{server['ORMMetaData']['common_result_update']}*/
...
const deleteRecord = async (app_id, resource_id) => {
    return server.ORM.Execute({app_id:app_id, dml:'DELETE', object:'IamAppAccess', delete:{resource_id:resource_id, data_app_id:null}}).then((/**@type{server['ORMMetaData']['common_result_delete']}*/result)=>{
        if (result.affectedRows>0)
            return {result:result, type:'JSON'};
        else
            return server.ORM.getError(app_id, 404);
