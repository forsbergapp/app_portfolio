 ...
 */
const get = (app_id, resource_id, res) => fileDBGet(app_id, 'IAM_USER',resource_id, null, res);
...
const post = async (app_id, data, res) => {
    ...
    if (!data.username || !data.password ||
        ...
        return fileDBPost(app_id, 'IAM_USER', data_new, res).then(()=>{return {id:data_new.id};});
    }
};
/**
 * Update
...
const update = async (app_id, resource_id, data, res) => {
    ...
    const user = fileCache('IAM_USER')
                .filter((/**@type{server_db_file_iam_user}*/row)=>row.id==resource_id)[0];
    if (user){
        if (user.username == data.username && data.password && await securityPasswordCompare(data.password, user.password)){
            ...
            if (Object.entries(data_update).length>0)
                return fileDBUpdate(app_id, 'IAM_USER', resource_id, null, data_update, res);
            else{
                res.statusCode = 404;
                throw '⛔';    
            }
...
const deleteRecord = async (app_id, resource_id, res) => {
    return fileDBDelete(app_id, 'IAM_USER', resource_id, null, res);
};
