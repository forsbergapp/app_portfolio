...
const post = async parameters => {
    ...
    if (  parameters.data.data_app_id==null ||
        ...
        return fileDBPost(parameters.app_id, 'APP_DATA_RESOURCE_MASTER', data_new).then((result)=>{
...
const update = async parameters =>{
   ...
   if (parameters.resource_id==null || parameters.data.data_app_id==null){
       ...
       if (Object.entries(data_update).length>0)
           return fileDBUpdate(parameters.app_id, 'APP_DATA_RESOURCE_MASTER', parameters.resource_id, null, data_update).then((result)=>{
               if (result.affectedRows>0)
                   return {result:result, type:'JSON'};
               else
                   return dbCommonRecordError(parameters.app_id, 404);
...
const deleteRecord = async parameters =>{
    return fileDBDelete(parameters.app_id, 'APP_DATA_RESOURCE_MASTER', parameters.resource_id, null).then((result)=>{
        if (result.affectedRows>0)
            return {result:result, type:'JSON'};
        else
            return dbCommonRecordError(parameters.app_id, 404);
