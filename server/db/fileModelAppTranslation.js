...
const get = (app_id, resource_id, locale, data_app_id) =>{
    ...
    const SPLIT = '-';
    const result = fileDBGet(app_id, 'APP_TRANSLATION',resource_id, data_app_id).rows.filter((/**@type{server_db_file_app_translation}*/row)=>row.app_id == data_app_id);
    if (result.filter((/**@type{server_db_file_app_translation}*/row)=>row.locale == locale)[0]){
        //return found for requested locale
        return {result:result.filter((/**@type{server_db_file_app_translation}*/row)=>(row.locale == locale)), type:'JSON'};
    }        
    else
        if (locale?.split(SPLIT).length==3 && result.filter((/**@type{server_db_file_app_translation}*/row)=>row.locale == (locale.split(SPLIT)[0] + SPLIT + locale.split(SPLIT)[1]))[0]){
            ...
            return {result:result.filter((/**@type{server_db_file_app_translation}*/row)=>row.locale == (locale.split(SPLIT)[0] + SPLIT + locale.split(SPLIT)[1])), type:'JSON'};
        }            
        else
            if (locale?.split(SPLIT).length==2 && result.filter((/**@type{server_db_file_app_translation}*/row)=>row.locale == locale.split(SPLIT)[0])[0]){
                ...
                return {result:result.filter((/**@type{server_db_file_app_translation}*/row)=>row.locale == locale.split(SPLIT)[0]), type:'JSON'};
            }
            else
                if (result.filter((/**@type{server_db_file_app_translation}*/row)=>row.locale == 'en')){
                    //return found for default language
                    return {result:result.filter((/**@type{server_db_file_app_translation}*/row)=>row.locale == 'en'), type:'JSON'};
                }
                else
