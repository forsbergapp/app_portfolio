...
const getResult = async parameters => {
    return {result:{resource:(await fs.promises.readFile(server.ORM.serverProcess.cwd() + `/data/${server.ORM.db.OpenApi.getViewConfig({app_id:0, data:{parameter:'SERVER_PATH_JOBS'}}).result}/${parameters.resource_id}.html`)).toString()}, 
            type:'JSON'};
};
/**
 * @name post
...
const postResult = async (app_id, id, result) =>{
    await fs.promises.writeFile(server.ORM.serverProcess.cwd() + `/data/${server.ORM.db.OpenApi.getViewConfig({app_id:0, data:{parameter:'SERVER_PATH_JOBS'}}).result}/${id}.html`, result,  'utf8');
