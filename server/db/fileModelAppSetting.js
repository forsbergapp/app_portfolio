/** @module server/db/fileModelAppSetting */

/**
 * @import {server_db_common_result_insert, server_db_common_result_update, server_db_common_result_delete, server_server_response,server_db_file_app_setting} from '../types.js'
 */
/**@type{import('./file.js')} */
...
const post = async (app_id, data) => {
    ...
    if (app_id!=null && data.app_id!=null && data.common_type!=null && data.common_name!=null && data.common_role!=null && data.common_path!=null){
        ...
        return fileDBPost(app_id, 'APP_SETTING', data_new).then((result)=>{
 ...
 * @param {{app_id:number,
 *          resource_id:number,
 *          data:server_db_file_app_setting}} parameters
...
const update = async parameters => {
    ...
    if (Object.entries(data_update).length>0)
        return fileDBUpdate(parameters.app_id, 'APP_SETTING', parameters.resource_id, null, data_update).then((result)=>{
            if (result.affectedRows>0)
                return {result:result, type:'JSON'};
            else
                return dbCommonRecordError(parameters.app_id, 404);
...
const deleteRecord = async (app_id, resource_id) => {
    return fileDBDelete(app_id, 'APP_SETTING', resource_id, null).then((result)=>{
        if (result.affectedRows>0)
            return {result:result, type:'JSON'};
        else
            return dbCommonRecordError(app_id, 404);
