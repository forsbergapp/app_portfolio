...
const get = (app_id, res) => {
    const result = fileDBGet(app_id, 'APP_SECRET',null, app_id);
    if (result.length>0)
        return result;
    else
...
const update = async (app_id, resource_id, data, res) => {
    if  (data.parameter_name=='app_id'){
        ...
        return fileDBUpdate(app_id, 'APP_SECRET', null, resource_id, {[data.parameter_name]:data.parameter_value}).then((result)=>{
