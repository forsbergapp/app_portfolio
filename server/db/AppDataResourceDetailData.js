...
const get = parameters =>{ 
    const result = fileDBGet(parameters.app_id, 'AppDataResourceDetailData',parameters.resource_id, parameters.data.data_app_id??null);
...
const post = async parameters => {
 ...
 if (  parameters.data.app_data_resource_detail_id==null){
     ...
     return fileCommonExecute({app_id:parameters.app_id, dml:'POST', object:'AppDataResourceDetailData', post:{data:data_new}}).then((result)=>{
...
const update = async parameters =>{
  ...
  if (parameters.resource_id==null){
      ...
      if (Object.entries(data_update).length>0)
          return fileCommonExecute({app_id:parameters.app_id, dml:'UPDATE', object:'AppDataResourceDetailData', update:{resource_id:parameters.resource_id, data_app_id:null, data:data_update}}).then((result)=>{
              if (result.affectedRows>0)
                  return {result:result, type:'JSON'};
              else
                  return getError(parameters.app_id, 404);
...
const deleteRecord = async parameters =>{
    return fileCommonExecute({app_id:parameters.app_id, dml:'DELETE', object:'AppDataResourceDetailData', delete:{resource_id:parameters.resource_id, data_app_id:null}}).then((result)=>{
        if (result.affectedRows>0)
            return {result:result, type:'JSON'};
        else
            return getError(parameters.app_id, 404);
