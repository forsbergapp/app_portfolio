...
const getParameters_server = (req, res) => {
	service.getParameters_server(getNumberValue(req.query.app_id), getNumberValue(req.params.app_id), (err, results) =>{
		if (err) {
...
const getParametersAllAdmin = (req, res) => {
	service.getParametersAllAdmin(getNumberValue(req.query.app_id), getNumberValue(req.params.app_id), req.query.lang_code, (err, results) =>{
		if (err) {
			return res.status(500).send({
				data: err
			});
...
const setParameter_admin = (req, res) => {
	...
	service.setParameter_admin(getNumberValue(req.query.app_id), data, (err, results) =>{
		if (err) {
			return res.status(500).send({
				data: err
			});
...
const setParameterValue_admin = (req, res) => {
	const data = {	app_id: 			getNumberValue(req.body.app_id),
					parameter_value: 	req.body.parameter_value,
					parameter_name: 	req.body.parameter_name};
	service.setParameterValue_admin(getNumberValue(req.query.app_id), data, (err, results) =>{
		if (err) {
			return res.status(500).send({
				data: err
			});
