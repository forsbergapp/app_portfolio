 ...
 const serverExpressApps = async (app) => {
    ...
    app.get('*',(/** @type{Types.req}*/req, /** @type{Types.res}*/res, /**@type{function}*/next) => {
		...
		if (app_id == null)
            ...
            if (req.originalUrl.toLowerCase().startsWith('/css')||
                req.originalUrl.toLowerCase().startsWith('/images')||
                req.originalUrl.toLowerCase().startsWith('/js')||
                req.originalUrl == '/manifest.json'||
                req.originalUrl == '/sw.js'){
                switch (req.originalUrl.toLowerCase().substring(req.originalUrl.lastIndexOf('.'))){
                    case '.css':{
                        res.type('text/css');
                        break;
                    }
                    ...
                    default:{
                        LogAppE(app_id, COMMON.app_filename(import.meta.url), 'app.get("/")', COMMON.app_line(), `Invalid file type ${req.originalUrl}`)
                        .then(()=>{
                            res.statusCode = 403;
                ...
                res.sendFile(`${process.cwd()}${ConfigGetApp(app_id,'PATH')}${req.originalUrl}`);
            }
            else
